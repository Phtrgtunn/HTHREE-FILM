        ...
        <div class="flex-1 overflow-y-auto">
          <div class="max-w-4xl px-8 py-8">
            <main class="w-full">
              ...
              <div class="flex items-center justify-between mb-4">
                <div>
                  <span class="inline-block bg-yellow-400 text-black text-xs font-bold px-3 py-1 rounded-full">
                    Thành viên từ tháng {{ getJoinMonth() }}
                  </span>
                </div>
                <button @click="router.push('/pricing')" class="text-yellow-400 hover:text-yellow-300 text-sm font-medium flex items-center transition-colors">
              ...
              <div v-else-if="!loadingSubscription && !activeSubscription && pendingSubscriptions.length === 0" class="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-lg p-6 mb-6 text-center">
                ...
                <h3 class="text-xl font-bold text-white mb-2">Chưa có gói đăng ký</h3>
                <p class="text-gray-400 mb-4">Đăng ký gói để trải nghiệm đầy đủ tính năng</p>
                <button @click="router.push('/pricing')" class="bg-gradient-to-r from-yellow-400 to-yellow-500 text-black font-bold px-6 py-3 rounded-lg hover:from-yellow-500 hover:to-yellow-600 transition-all">
                  Xem các gói đăng ký
                </button>
              </div>
...
const getRealtimeProgress = (sub) => {
  ...
  const progress = totalMs > 0 ? Math.min(100, Math.max(0, (usedMs / totalMs) * 100)) : 0;
...
const saveProfile = async () => {
  ...
  try {
    ...
    if (storedUser) {
      ...
      console.log('👤 Updated user name in localStorage:', profileForm.displayName);
    }
